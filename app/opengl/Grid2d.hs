...
main = do
  ...
  -- reference var grid visible flag
  stateRef <- newIORef (State True) :: IO (IORef State)
  ...
  -- set callbacks
  displayCallback       $= display gridRef csRef stateRef
  motionCallback        $= Just (motion msRef gridRef)
  keyboardMouseCallback $= Just (keyboardMouse msRef gridRef stateRef view2model)
...
overwrite :: MVar Model.Command -> Model.Command -> IO ()
overwrite mvar v = do
...
toggle gridRef cmdVar (Position px py) = do
  ...
  overwrite cmdVar $ Model.ToggleCell c

keyboard :: MVar Model.Command -> IORef State -> Char -> IO ()
keyboard v2m state c =
  case c of
    's' -> overwrite v2m Model.SingleStep
    'r' -> overwrite v2m Model.Run
    'g' -> do
      state $~ (\(State b) -> State (not b))
      postRedisplay Nothing
    'c' -> overwrite v2m Model.Clear
    _   -> return ()

keyboardMouse :: IORef (Maybe MotionState) -> IORef Grid -> IORef State
  -> MVar Model.Command -> KeyboardMouseCallback
keyboardMouse msRef gridRef _ _ (MouseButton LeftButton) Down
...
keyboardMouse msRef g _ cmdVar (MouseButton LeftButton) Up _ pos = do
  ms <- get msRef
  modifyIORef msRef $ const Nothing
  when (isJust ms) $ do
    let MotionState _ _ _ clicked = fromJust ms
    when clicked  $ toggle g cmdVar pos
keyboardMouse _ _ stateRef v2m (Char c) Down _ _ = keyboard v2m stateRef c
...
prepareModelview :: Grid -> [(Int,Int)] -> Bool -> IO ()
prepareModelview (Grid left top sc) cs showGrid = do
  clear [ColorBuffer]
  ...
  when showGrid $ drawGrid l r b t

  -- filter cells that fall in viewing volume
  let cs' = filter (\(x,y) -> l <= x && x <= r && b <= y && y <= t) cs
  mapM_ drawRect cs'

display :: IORef Grid -> IORef [(Int,Int)] -> IORef State -> DisplayCallback
display gridRef csRef stateRef = do
  -- update grid
  g <- get gridRef
  ...
  cs <- get csRef
  State b <- get stateRef
  prepareModelview g cs b
